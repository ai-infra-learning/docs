...
本章可以理解为一章“CUDA 性能优化入门地图”。它不是单独讲 CUDA 语法，也不是单独讲 GPU 硬件，而是把 GPU 架构、CUDA 编程模型、内存体系和性能分析方法串联起来。原文先解释 GPU 与 CPU 的差异：CPU 更追求单线程低延迟，而 GPU 更追求大量线程同时执行的吞吐量；随后介绍 GPU 内部的 SM、warp scheduler、SIMT 执行模型，以及 thread、warp、block、grid 这些 CUDA 线程层级；接着讲解如何选择 `threadsPerBlock` 和 `blocksPerGrid`，为什么线程块大小通常要是 32 的倍数，以及为什么 kernel 中必须做边界检查；中间部分重点介绍 GPU 内存体系，包括 register、shared memory / L1、L2、HBM、TMEM、TMA、Unified Memory、异步内存分配和 memory pool；最后通过 occupancy、launch bounds、CUDA Occupancy API、Compute Sanitizer、Roofline 模型、Nsight Systems、Nsight Compute 和 NVTX，说明如何判断一个 CUDA kernel 是并行度不足、资源受限、内存受限还是计算受限。整章的主线可以概括为：高性能 CUDA 编程的关键，是让 GPU 有足够多的有效并行工作，同时让数据尽可能靠近计算单元，并用 profiling 工具验证每一次优化是否真正解决了瓶颈。
...
这一部分对应原文前半部分的架构基础，重点回答三个问题：GPU 为什么快，SM 和 warp scheduler 如何工作，以及 CUDA 为什么要用 thread、warp、block、grid 这样的层级来组织并行任务。

#### 1.1 GPU 为什么快：吞吐量优先，而不是单线程优先

理解本章内容，需要先区分 CPU 和 GPU 的设计目标。CPU 的设计更偏向“尽快完成一个复杂任务”，它依赖较强的单线程性能、复杂的分支预测、乱序执行和大缓存，因此适合执行控制流复杂、线程数量不多、每一步逻辑差异较大的任务。
...
开头用 CUDA 的基本执行流程说明 CPU 和 GPU 如何配合：
...
这一流程说明，GPU 程序的性能问题不一定只来自 kernel 本身，也可能来自 CPU-GPU 数据传输，或者来自数据传输和计算没有重叠。后文中 `cudaMemcpyAsync`、pinned memory、Nsight Systems 时间线等内容，都与这一问题相关。
...
| 内存            | 16GB HBM2   | 40/80GB HBM2e | 80GB HBM3   | 192GB HBM3e |
...
原文重点介绍了 Blackwell 中 SM 的组织方式和 warp 调度机制。
...
原文还介绍了 Blackwell SM 内部的 warp scheduler。一个 SM 可以看作由多个调度分区组成，每个分区有自己的 warp scheduler 和 dispatch logic。理想情况下，四个 scheduler 每个周期可以从四个不同 warp 中各发射一条指令。
...
还有特殊函数单元SFU，也就是 Special Function Unit。SFU 用于处理 `sin`、`cos`、倒数、平方根等特殊函数。SFU 拥有独立流水线，不属于普通 math + memory 的 dual issue 配对。这个细节说明 GPU 内部并不是单一计算单元，而是包含 INT32、FP32、Tensor Core、LD/ST、SFU 等多种执行管线。

这一节的核心结论是：

> GPU 架构层面的关键，是 SM 能同时维护大量 warp，并通过 warp scheduler 把 ready warp 持续送入不同流水线执行。后文中的 occupancy，本质上就是讨论如何让 SM 有足够多的 warp 可以调度。
...
原文将在 第 8 章 中更深入探讨 warp 发散，以及检测、分析和缓解它的方法。

（需要注意的是:
- 在Volta 架构之前，Warp 内的线程必须一起执行
- Volta 改进了该机制，允许 Warp 内的线程独立等待/执行
- 这样能够更加灵活处理 锁，同步，条件执行）
...
原文还补充了现代 GPU 的 thread block cluster（此特性在Hopper 架构引入）。之前的 CUDA 中，不同 block 之间通常不能直接协作，因为 block 的执行顺序不保证，也不一定在同一个 SM 上。但现在 GPU 支持把多个 block 组成 cluster，让它们跨 SM 通信。

这里涉及 DSMEM，也就是 distributed shared memory。它把参与 cluster 的多个 SM 的 shared memory 通过片上互连连接起来，使不同 block 可以访问彼此的 shared memory，并使用 cluster 级别的 barrier。（原文说明 DSMEM 和 thread block cluster 会在第10章涵盖诸如线程块簇和 DSMEM 之类的高级主题进一步展开，本章只先建立概念。）
...
这一部分对应原文中 CUDA kernel 编写、启动参数以及多维输入映射部分。核心问题是：如何把数据任务映射到 CUDA 线程层级上，并让 kernel 启动配置符合硬件执行方式。

#### 2.1 选择 threadsPerBlock 和 blocksPerGrid

启动参数是编写 CUDA kernel 最先遇到的问题。原文强调，`threadsPerBlock` 通常要选择 32 的倍数。原因很直接：warp 大小固定为 32。如果 block size 不是 32 的倍数，就会产生不完整 warp，浪费了可以进行并行的资源。
...
因此，原文建议常见起点是 `threadsPerBlock = 256`。这个配置有 8 个 warp，通常能在 occupancy 和资源使用之间取得较好平衡。根据 kernel 的寄存器使用、shared memory 使用和 profiling 结果，也可以尝试 128、512 等配置。

原文还给出了 Blackwell B200 的一些硬件限制：
...
这里的 `if (idx < N)` 非常关键。如果没有这个检查，多出来的线程可能访问数组边界外的地址，引发非法内存访问。

原文还提到 CUDA grid 本身也有维度限制，例如 X 维最多可到 2,147,483,647 个 block，Y/Z 维最多 65,535 个 block，并且一个设备最多可以有 128 个 concurrent grids。不过实际开发中，通常更先碰到的是 thread、block、register、shared memory 这些资源限制。
...
原文还提醒，CUDA kernel 是异步执行的。CPU 启动 kernel 后不会自动等待 GPU 执行完。如果 kernel 中发生越界访问，错误也可能不会立刻显示，而是在后续同步或 CUDA API 调用时才出现。因此开发时通常需要检查：
...
这里每个 block 有 `16 x 16 = 256` 个线程，仍然符合前文提到的常见起点。kernel 内部可以计算二维坐标：
...
如果是 3D 体数据，也可以使用 `dim3(x, y, z)` 把线程映射到三维空间。原文说本书大部分场景会使用 1D 或 2D tiled 配置，但理解 3D 映射有助于处理体数据、三维网格等任务。

### 3 GPU 内存体系与数据移动

这一板块对应原文中关于内存管理、内存层级、TMEM/TMA 和 Unified Memory 的内容。GPU 性能优化的核心往往不是“多算几次”，而是“少等数据”。

#### 3.1 异步内存分配、memory pool 和 stream

原文指出，标准的 `cudaMalloc` 和 `cudaFree` 是同步且相对昂贵的。这里的“同步”可以理解为：为了保证内存分配或释放是安全的，CUDA runtime / driver 可能需要等待设备上相关工作完成，甚至引入全设备级别的同步。这样一来，本来可以继续排队执行的 GPU 工作会被打断，CPU 端也可能被迫等待。

除了同步开销，传统分配释放还涉及驱动和操作系统级调用。这种交互会引发用户态到内核态的上下文切换，以及 NVIDIA 驱动内部的显存资源管理开销。原文中提到的 `mmap` / `ioctl` 就属于这类底层系统调用：`mmap` 常用于建立内存映射，`ioctl` 常用于向设备驱动发送控制命令。它们不是普通函数调用，而是会进入操作系统内核和驱动层，因此比纯设备端操作更慢。
...
stream 可以理解为一个有序队列。同一个 stream 内的操作按顺序执行，比如先分配、再拷贝、再启动 kernel、再释放。不同 stream 之间则有机会并发或重叠。因此，`cudaFreeAsync(d_buf, s)` 并不是立即粗暴地释放内存，而是表示：等到这个 stream 中前面依赖 `d_buf` 的工作完成后，再把这块内存释放回池子。
...
`cudaMallocAsync` 和 `cudaFreeAsync` 底层使用 CUDA memory pool。memory pool 可以理解为显存缓存池：第一次需要某个大小的 buffer 时，CUDA 可能仍然需要向底层驱动申请显存；但释放时，这块显存不一定立刻还给系统，而是回到 pool 中。后面再次申请相近大小的 buffer 时，可以直接从 pool 中复用。
...
类似的在 PyTorch 中也不会每次 tensor 生命周期结束就立刻把显存还给系统，而是缓存起来复用。这样训练循环中反复创建 tensor 时，就不会每次都触发昂贵的底层 `cudaMalloc`。
...
不过也需要注意，memory pool 并不是让显存消耗凭空减少。它更像是“缓存已申请的显存以便复用”。因此在监控工具中可能看到程序占用的显存没有马上下降，这是正常现象。它换来的是更低的分配开销和更稳定的延迟。如果确实需要把显存还给系统，可以通过 CUDA memory pool 的相关属性或 trim 操作进行控制，原文也提到可以调节 release threshold 等策略。
...
原文用较大篇幅讲 GPU memory hierarchy，这是本章非常重要的部分。这里最容易混淆的是：有些名称来自 CUDA 编程模型，有些名称来自硬件结构图。读书会中建议先把“逻辑视角”和“物理视角”分开。
...
这里还需要补充一点：local memory 通常不是程序员主动使用的。多数 CUDA 代码里不会显式写“使用 local memory”，它更多是 CUDA 编译器在寄存器放不下线程私有数据时自动生成的“备用空间”。例如，一个线程里有太多局部变量、线程私有数组太大、数组索引无法在编译期确定，或者复杂函数调用产生很多临时值时，编译器可能无法把所有数据都放进 register，于是把一部分数据 spill 到 local memory。因此，平时写 CUDA 程序时很少“主动使用 local memory”是正常的。但在性能优化时必须关注它，因为它可能悄悄出现并拖慢 kernel。可以通过编译参数 `-Xptxas -v` 查看是否出现 `spill loads` / `spill stores`，也可以在 Nsight Compute 中观察 local memory load/store、registers per thread 和 spilling 相关指标。
...
TMEM 是 Blackwell 中与 Tensor Core 路径相关的片上存储。它不是普通 CUDA C++ 指针可以直接读写的空间，而是服务 Tensor Core 指令和矩阵乘加累加路径。本章关注现代 AI 计算时，Tensor Core 数据路径非常关键。

L2 cache 是全 GPU 共享缓存，位于所有 SM 和 HBM 之间。可以把它理解成一个全局缓冲层：当某个 SM 从 global memory 读取数据时，数据可能先经过 L2；如果后续其他 SM 或同一个 SM 再访问相同或相邻的数据，就可能从 L2 中命中，而不必重新访问 HBM。原文提到 Blackwell 级别 L2 约 126 MB，这个容量比每个 SM 内部的 shared memory / L1 大很多，因此更适合做跨 SM 的数据复用缓冲。
...
HBM 全称是 High Bandwidth Memory，即高带宽内存。它容量大、带宽高，但相对于 register、shared memory、L1/L2 cache 来说，延迟仍然高。原文以 Blackwell B200 为例，提到 HBM3e 容量可到约 192 GB，带宽约 8 TB/s。这个带宽听起来非常高，但 GPU 的 Tensor Core 计算吞吐也非常高，因此很多时候问题不是“算不动”，而是“数据来不及送到计算单元”。

通过 L2，可能被复用，从而减少再次访问 DRAM 的机会。为最大化 L2 的好处，应将全局加载组织为与缓存行整洁映射的128 字节对齐、合并（coalesced）事务。
...
原文还专门提到 Blackwell 中的 TMEM 和 TMA。这部分比较偏新架构，但原文有写，因此分享笔记中需要覆盖。

TMEM 是 Tensor Memory，可以理解为 Blackwell 每个 SM 上专门服务 Tensor Core 的片上内存。原文提到 TMEM 每个 SM 大约 256 KB，用于第五代 Tensor Core 指令，比如 `tcgen05.*` 和 UMMA。
...
这一部分对应原文中 occupancy、GPU 利用率、launch bounds、Occupancy API 以及 Compute Sanitizer 的内容。它关注两个问题：如何让 GPU 有足够多的有效工作，以及如何保证 CUDA 程序本身是正确的。
...
但原文也强调，occupancy 很重要，但并不是越高越好。真正目标是提高吞吐量，而不是单纯把 occupancy 数字堆到 100%。

以长度为N的两个向量求和`C = A + B` 做例子。错误版本 `add_sequential` 只用一个线程或一个 warp 处理所有元素：
...
这里要强调一个容易混淆的点：GPU 是面向吞吐量优化的处理器，不是为了让少数线程跑得尽可能快，而是为了让大量线程同时推进。CPU 负责发起 kernel，GPU 内部的 SM、warp scheduler、计算单元和内存子系统共同执行 kernel。这个过程中会不断发生访存、cache 访问、指令依赖等等待。如果只有很少线程，这些等待会直接变成硬件空闲；如果有足够多 warp，SM 就可以在 warp 之间切换，用其他 warp 的计算或访存来覆盖等待时间。

因此，这个例子说明了 GPU 优化的第一步：先给 GPU 足够多的并行工作。无论单个线程写得多快，如果线程数量太少，就无法利用 GPU 的吞吐量潜力。

需要注意的是，并行版本的 achieved occupancy 只有约 38.7%，并不是 100%，但性能已经大幅提升。这正好对应原文的观点：occupancy 不是越高越好。只要活跃 warp 足够隐藏主要延迟，性能瓶颈就可能转移到别处。后续优化才会进入更细的层面，例如提高每个 warp 内部的指令级并行性、减少指令依赖、改善内存访问模式，或者提高 Tensor Core / CUDA Core 的实际利用率。
...
这里的 `__launch_bounds__(256, 16)` 表示两层含义：
- `256` 表示承诺这个 kernel 启动时，每个 block 的线程数不会超过 256。
- `16` 表示希望每个 SM 至少可以常驻 16 个 block。
`launch_bounds` 的第一个参数是实际 `threadsPerBlock` 的上限，第二个参数不是 `blocksPerGrid`

这个注解会影响编译器的寄存器分配、函数内联和循环展开策略。它的目的不是突破硬件上限，而是让编译器在“单个线程使用更多资源”和“让更多 warp 常驻 SM”之间做取舍。现代 NVIDIA GPU 每个 SM 的常驻 thread 数量有上限，例如 2048。如果请求 `16 x 256 = 4096` 个线程常驻一个 SM，就超过了硬件能力，编译器会降低请求并可能给出 warning。
...
```cpp
gridSize = max(minGridSize, ceil_div(N, bestBlockSize));
```
...
NVIDIA Compute Sanitizer是随CUDA Toolkit提供的功能正确性套件，它通过在运行时对代码进行插桩来在开发早期发现错误。
...
Compute Sanitizer 主要包含四类工具，它们有助于检测越界内存访问、数据竞争、未初始化内存读取以及CUDA代码中的同步问题：
...
这一部分对应原文最后的 Roofline、Nsight 工具和总结部分。

#### 5.1 Roofline 模型：判断 memory-bound 还是 compute-bound

Roofline 模型是一种有用的可视化方法，这个模型用于回答一个关键问题：kernel 慢，到底是算不动，还是数据搬不动？
...
关键指标是 算术强度（arithmetic intensity），也就是每传输 1 字节数据能做多少 FLOP。
​
$$
...
然后使用 Roofline 和 profiling 工具结合起来。
...
Nsight Systems 会在时间线上显示这些 NVTX 区间，使得观察重叠变得容易。使用异步内存拷贝（cudaMemcpyAsync）时，数据传输会与 GPU 上的内核执行重叠，比较同步和异步内存传输。
...
需要提醒的是，`cudaMemcpyAsync` 可以实现和 kernel 重叠。要真正重叠，通常需要：
- 使用正确的 CUDA stream。
...
最后，高性能 CUDA 编程不是单纯追求某一个指标，而是在并行度、资源占用、内存层级和实际吞吐之间找平衡。先保证程序正确，再通过 Roofline、Nsight Compute、Nsight Systems 和 NVTX 找到瓶颈，最后用 profiling 验证每一次优化是否真正有效。

## 核心要点

最后对本章进行了总结。可以把本章收束为以下几个关键点。
...
第四，occupancy 受资源限制影响。每个线程使用的 register、每个 block 使用的 shared memory、SM 最大 resident warp 数和 resident block 数，都会限制同一时刻能驻留的线程数量。Blackwell 相关原文中提到的限制包括每线程最多 255 个 register、每 SM shared memory 上限约 228 KB、最大 resident warp 数 64、最大 resident thread block 数 32。
...
第八，Roofline 模型用于判断优化方向。arithmetic intensity 决定 kernel 更接近 memory-bound 还是 compute-bound。对于 memory-bound 场景，可以通过低精度、压缩、提高数据复用、改善访存模式等方式减少数据移动。TMEM 与 Blackwell 的 统一矩阵乘加（UMMA） 也是改善矩阵计算中的数据供给路径。
