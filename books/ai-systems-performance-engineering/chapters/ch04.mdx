...
## FAQ
  ...
  构造方法来自 Sanders, Speck & Träff (2009)，对 2^k 个 rank
  ...
  创建、内存注册）、控制面操作（bootstrap、barrier、health check，走 TCP，数据量极小）、传统模式下的 NCCL kernel 调度（2.28 Device
  API 可消除）、数据加载与预处理。

  DDP 的 autograd hook 也在 CPU 上，但它做的是轻量调度（标记梯度 ready、检查 bucket、提交 NCCL
  调用），微秒级开销。理论上，在静态图场景下（torch.compile / CUDA Graphs），backward 执行顺序和 bucket
  映射都是固定的，可以把就绪判断做成 GPU 端原子计数、通信发起走 Device API，完全不需要 CPU 介入。但动态图（eager mode）下 autograd
  ...
  **线上不方便挂 profiler 时**，可以用轻量方式定位：在 all-reduce 前后各插一个 `torch.cuda.synchronize()` +
  ...
  也可以不改代码，直接看 `nvidia-smi dmon -s u` 的 GPU 利用率波动——straggler 所在节点的 GPU util
  会周期性出现短暂低谷（在等上游计算完成），而其他节点的低谷出现在 collective 等待阶段。网卡侧用 `ethtool -S` 或 InfiniBand
  ...
  数据并行（DDP）按 GPU 数量均分 mini-batch，这是最直观的切法。但其他并行策略切的不是数据，而是模型或计算本身，且不一定均分：

  - **张量并行（TP）**：切层内权重矩阵。例如一个 `[4096, 4096]` 的线性层，TP=4 时每张 GPU 持有 `[4096,
  1024]`，按矩阵维度切分，不依赖 GPU 总数。
  - **流水线并行（PP）**：按层分段。各段的层数可以不同——如果前几层计算量更大，可以分给更多 GPU（不均匀切分）。
  - **专家并行（EP）**：MoE 的 expert 分散到不同 GPU。每张 GPU 上的 expert 数量可以不同，且每个 token 只路由到少数
  expert，负载天然不均匀。
  - **ZeRO / FSDP**：切参数 + 优化器状态 + 梯度。切分粒度由 ZeRO stage 决定（Stage 1 只切优化器状态，Stage 3 切所有），与 GPU
  数量无关。
  - **序列并行（SP）**：切长序列。每张 GPU 处理序列的一个片段，片段长度可以按计算量调整。

  实践中多种策略组合使用（3D/4D 并行），同一组 GPU
  可能在不同维度上用不同的切法和不同的并行度。切分方式的选择取决于模型结构、硬件拓扑和通信开销的权衡。
  </Accordion>

  <Accordion title="用真实请求做推理 replay profiling，会有用户数据隐私问题吗？">
  有潜在风险。Nsight Systems trace 通常只记录 kernel 名称、时长和内存地址，不记录 tensor 内容，PII 风险相对低。但自定义日志或
  debug print 可能泄漏实际数据。建议用合成数据或脱敏数据做性能 benchmark；如果必须用真实数据，确保在安全环境中操作，trace
  文件按敏感数据处理。
  </Accordion>
...
- [Two-tree algorithms for full bandwidth broadcast, reduction and scan](https://dl.acm.org/doi/10.1016/j.parco.2009.08.003) — Sanders, Speck & Träff (2009)，double binary tree 的理论基础
