...
节点内多张 GPU 走 NVLink/NVSwitch 直连即可；要让**不同节点**的 GPU 以全 NVLink 带宽共享显存，需要 Multi-Node NVLink（MNNVL）和 IMEX。NVIDIA DRA driver 的另一子系统 [ComputeDomain](https://dra-driver-nvidia-gpu.sigs.k8s.io/docs/)（`compute-domain.nvidia.com`）就是为此设计：它按工作负载临时拉起一条跨节点的 NVLink fabric（IMEX 通道），随作业创建、随作业销毁。和仍处于 Technology Preview 的 GPU 分配侧不同，ComputeDomain 是 NVIDIA DRA driver 中**已正式支持**的部分。

前置条件：

- **MNNVL 硬件**：多个节点接入同一 NVLink 交换 fabric，典型是 GB200 NVL72。普通以太网 / InfiniBand 互联的双机不算。
- NVIDIA Driver `v570.158.01+`；GPU Feature Discovery 给 MNNVL 节点打 `nvidia.com/gpu.clique` 标签；节点上禁用 `nvidia-imex.service`。

用法是先建一个 `ComputeDomain`，它会自动生成一个 `ResourceClaimTemplate`，Pod 再引用其中的 channel：
...
Pod 通过 `resourceClaimTemplateName: imex-channel-0` 申请到 channel 后，容器里会注入 `/dev/nvidia-caps-imex-channels/...`，从而跨节点走 NVLink。

<Tip>
ComputeDomain 只适用于真正的 MNNVL 系统（GB200 NVL72 等）。单张卡、单节点多卡、或普通网络互联的多机都用不上；这类场景下安装 driver 时可 `--set resources.computeDomains.enabled=false` 关掉该子系统。
</Tip>
