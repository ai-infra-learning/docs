...
这个功能还依赖 `nvidia-cuda-mps-control` 本身支持 `-S`、`sm_partition` 和 `lspart`。如果 `nvidia-cuda-mps-control --help` 里没有这些选项或命令，说明当前 driver/MPS 工具版本不能运行 static SM partitioning。

[run_static_partition_demo.sh](/books/ai-systems-performance-engineering/chapters/ch03/mps/run_static_partition_demo.sh) 会先跑一轮 Basic MPS，再启动 `-S` 模式的 MPS control daemon 做 static SM partitioning。两轮都运行相同数量的 `mps_demo.py` client，最后对比 Basic MPS 和 static partitioning 完成的总矩阵乘法次数。

```bash
# 进入 ch03 代码目录
cd books/ai-systems-performance-engineering/chapters/ch03

GPU_INDEX=0 RUN_SECONDS=60 N=1024 PARTITION_CHUNKS="7 7 7 6" ./mps/run_static_partition_demo.sh
```

`7 7 7 6` 表示为 `4` 个 client 创建 `4` 个大小接近的 partition，用来做接近平均的 SM 切分。
...
# CUDA_VISIBLE_DEVICES 控制当前进程可见的 GPU；0 表示只看到 GPU 0。
# 如果要设置多个 GPU，可以用逗号分隔，例如 0,1。
# 以 -S 模式启动 MPS control daemon，并让它只绑定 GPU 0。
sudo env CUDA_VISIBLE_DEVICES=0 nvidia-cuda-mps-control -d -S

# 查询目标 GPU 的 UUID。
GPU_UUID=$(nvidia-smi --query-gpu=uuid --format=csv,noheader -i 0)

# 查看当前 SM partition 状态。
echo "lspart" | sudo nvidia-cuda-mps-control

# 从 "Partition GPU-.../... created" 输出中提取 CUDA_MPS_SM_PARTITION 需要的分区 ID。
parse_partition() {
  sed -nE 's/^.*(GPU-[^[:space:]]+\/[^[:space:]]+).*$/\1/p' | tail -n 1
}

# 创建四个大小接近的 SM partition。
PART_0=$(echo "sm_partition add $GPU_UUID 7" | sudo nvidia-cuda-mps-control | parse_partition)
PART_1=$(echo "sm_partition add $GPU_UUID 7" | sudo nvidia-cuda-mps-control | parse_partition)
PART_2=$(echo "sm_partition add $GPU_UUID 7" | sudo nvidia-cuda-mps-control | parse_partition)
PART_3=$(echo "sm_partition add $GPU_UUID 6" | sudo nvidia-cuda-mps-control | parse_partition)

# CUDA client 初始化前设置 CUDA_MPS_SM_PARTITION，绑定到指定 partition。
CUDA_MPS_SM_PARTITION="$PART_0" uv run python mps/mps_demo.py --workers 1 --seconds 60

# 删除 partition 并关闭 MPS。
echo "sm_partition rm ${PART_0%/*} ${PART_0#*/}" | sudo nvidia-cuda-mps-control
echo quit | sudo nvidia-cuda-mps-control
```
...
```text
# echo "sm_partition add $GPU_UUID 7" | sudo nvidia-cuda-mps-control
Partition GPU-74d43ed3-cdf7-e667-3644-bf5b4f46ed65/Dx4AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA created

# echo "lspart" | sudo nvidia-cuda-mps-control
GPU           Partition                             free    used    free  used  clients
                                                    chunk   chunk    SM    SM
GPU-74d43ed3      -                                 0       27      0     108  -
GPU-74d43ed3  Dx4AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA  -       7       -     28   Yes
GPU-74d43ed3  Cx4AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA  -       7       -     28   Yes
GPU-74d43ed3  Bx4AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA  -       7       -     28   Yes
GPU-74d43ed3  Ax4AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA  -       6       -     24   Yes
```

`clients=Yes` 表示已有 client 使用该 partition；`used chunk` 是分配的 chunk 数，实际对应多少 SM 以 `used SM` 为准。
