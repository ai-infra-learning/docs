...
下面的例子使用配套脚本 [mps_demo.py](/books/ai-systems-performance-engineering/chapters/ch03/mps/mps_demo.py)。脚本会启动多个 Python 子进程，每个子进程在 GPU 上反复执行矩阵乘法，用来验证多个 CUDA 进程是否通过 MPS 共享同一张 GPU。

**Basic MPS**

`run_basic_comparison.sh` 会跑两轮：第一轮不开 MPS，第二轮启动 MPS 后再跑同一个 demo。两轮使用相同的 worker 数、矩阵大小和运行时长，最后用所有 worker 的总 `iters` 近似比较吞吐量。
...
# total_iters 是所有 worker 的 iters 之和；这里 MPS 版本约为不开 MPS 的 1.18 倍。
```

这段输出可以从几个关键信号判断：

- **吞吐提升**：这组 demo 中，开启 MPS 后总吞吐约提升 `18%`。两轮实验都使用 `4` 个 worker、`N=1024`、运行 `60` 秒，因此可以用所有 worker 的 `iters` 之和近似表示总吞吐。不开 MPS 时总共完成 `394603` 次矩阵乘法；开启 MPS 后完成 `464780` 次，`464780 / 394603 ≈ 1.18x`。
- **MPS client**：实际运行 CUDA work 的应用进程。这里就是 `4` 个 `python3` worker，它们真正发起矩阵乘法 kernel。`echo ps | sudo nvidia-cuda-mps-control` 能看到多个 client PID，并且它们的 `SERVER` 都是 `3817822`，说明这些 Python 进程连接到了同一个 MPS server。
- **MPS server**：中间代理进程，也就是 `nvidia-cuda-mps-server`。它代表多个 client 统一持有 GPU context，并把这些 client 的 GPU work 提交给 GPU。`nvidia-smi` 的进程表里能看到 `nvidia-cuda-mps-server`，说明这些 Python worker 正在通过 MPS server 使用 GPU。
