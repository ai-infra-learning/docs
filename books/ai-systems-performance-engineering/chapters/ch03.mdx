...
`run_basic_comparison.sh` 会先在不开 MPS 的情况下运行 demo，再启动 MPS 并重复运行同一个 demo。脚本会自动设置 `EXCLUSIVE_PROCESS`、启动 `nvidia-cuda-mps-control -d`、查看 MPS client 和 `nvidia-cuda-mps-server`，最后汇总两次运行的总 `iters`。
