...
用户基于 `DeviceClass` 声明要什么设备、要多少。有两种写法：直接建一个 `ResourceClaim`，或用 `ResourceClaimTemplate` 让 K8s 为每个 Pod 自动生成 claim。设备申请字段（`devices.requests`）两者完全一样，区别只在外层包装和 Pod 的引用方式。
...
Pod 提交到 API Server
    ...
    │        （直接引用已有 claim 则跳过这步）
    │
    ├─ 2. 调度器 Filter：遍历节点，基于 ResourceSlice 判断
    │        - 先用 DeviceClass 的 CEL 筛出符合 class 的设备
    │        - 再按 Claim 的 selector / 容量 / 约束进一步过滤
    │
    ├─ 3. 调度器 Reserve + Bind
    │        - 选定节点 + 选定具体设备
...
调度器不只选节点，还选定了**具体设备**。Reserve 阶段在调度器内部缓存里完成设备选择，Bind 阶段把结果写进 `ResourceClaim.status.allocation`，之后 driver 和 kubelet 都以这份结果为准。三个组件分工如下：
