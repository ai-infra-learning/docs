...
#### 6.3 Dynamic Resource Allocation (DRA)
...
与 device plugin 相比，这些收益显著改进了设备分配流程；device plugin 要求按容器请求设备，不支持设备共享，也不支持基于表达式的设备过滤。
...
<div style={{ maxWidth: "980px", margin: "0 auto" }}>
<Frame>
  <img src="/books/ai-systems-performance-engineering/images/ch03-dra-api-kinds-workflow.png" alt="Kubernetes DRA API kind 关系示意图，展示 DeviceClass、ResourceClaim、ResourceClaimTemplate、ResourceSlice 与 Pod 调度之间的关系" />
</Frame>
</div>
...
| --- | --- | --- |
| 资源模型 | 整数计数（`nvidia.com/gpu: N`） | 结构化对象（`DeviceClass` / `ResourceClaim` / `ResourceSlice`） |
| 设备选择 | 随机/首个可用 + 手动 `nodeSelector` | CEL 按属性筛（显存、架构、UUID 等） |
| 共享 | 支持 time-slicing / MPS，但按节点 ConfigMap 统一配置 | 支持 time-slicing / MPS，按 `ResourceClaim` 逐工作负载配置 |
| MIG | 需预切，节点级固定布局 | 可按 `ResourceClaim` 动态切（H100+） |
| 拓扑感知 | 有限（靠 Topology Manager） | 原生约束（同卡 `parentUUID`、NVLink/PCIe root） |
| 单卡维护 / 健康 | 坏卡要 drain 整节点；状态粗 | device taints；`ResourceSlice` 实时上报 |
| 成熟度 | 稳定、生态全 | GA v1.34，部分能力（动态 MIG 等）仍 alpha/beta |

DRA 把“设备能力、共享策略、拓扑约束、生命周期”都纳入声明式 API，由此带来按属性选设备、逐工作负载配置共享、动态切 MIG、表达设备互联拓扑等能力。它用四个对象分工协作：

| 对象 | 谁创建 | 作用 |
| --- | --- | --- |
| `DeviceClass` | 集群管理员 / driver | 定义一类可申请设备（`gpu.nvidia.com`、`mig.nvidia.com`） |
| `ResourceSlice` | DRA driver | 发布节点上有哪些设备及其属性，由 driver 自动维护 |
| `ResourceClaim` / `ResourceClaimTemplate` | 用户 / 工作负载 | 声明要什么设备（可带 CEL selector 和配置） |
| Pod | 用户 | 引用 `ResourceClaim` |

以下示例都在本书仓库 `books/ai-systems-performance-engineering/chapters/ch03/dra/` 下，可直接运行；环境用 `scripts/setup-dra-env.sh` 一键拉起（kind + DRA driver）。所有请求都遵循同一套路：`ResourceClaimTemplate`（或 `ResourceClaim`）引用一个 `DeviceClass`，再由 Pod 引用该 `ResourceClaim`。

##### GPU 分配与共享

最基础的用法是申请整卡：`ResourceClaimTemplate` 引用 `gpu.nvidia.com`，Pod 引用它即可拿到一张 GPU。
...
用 `ResourceClaimTemplate` 时每个 Pod 各生成一个独立 claim、拿到不同 GPU；改用一个共享的 `ResourceClaim`（Pod 以 `resourceClaimName` 引用），则多个 Pod 可共享同一张卡。`quickstart/` 提供三种拓扑示例：两 Pod 各独占一卡、一 Pod 两容器共享一卡、两 Pod 共享一卡。

##### TimeSlicing 与 MPS

DRA 把共享策略写进 claim 的 `GpuConfig`，**逐工作负载**配置（区别于 device plugin 的节点级 ConfigMap）。同一个模板里甚至能给不同 request 配不同策略：
...
这两种策略分别需要开启 `TimeSlicingSettings`、`MPSSupport` feature gate，否则 kubelet 在 `NodePrepareResources` 阶段会拒绝。完整示例见 `mps-timeslicing/`。

##### 静态 MIG

A100 等不支持运行时切换 MIG 模式的卡走这条路：先在节点上用 `nvidia-mig-parted` 预切，DRA 只负责把已存在的 MIG 设备分配出去。claim 用 `mig.nvidia.com` + CEL selector 按 profile 筛：
...
profile 名要和 `mig-parted` 切出来的一致；没有匹配的 MIG 设备时 Pod 会一直 Pending。示例见 `static-mig-a100/`。

##### 动态 MIG

H100 及更新架构可按 claim **现切** MIG，无需预切：开启 `DynamicMIG` feature gate 后，driver 按 `ResourceClaim` 在卡上动态创建 MIG。底层是 DRA 的 **partitionable devices**（[KEP-4815](https://github.com/kubernetes/enhancements/tree/master/keps/sig-scheduling/4815-dra-partitionable-devices)）：driver 把一张 GPU 的资源池声明成 `sharedCounters`，把所有可能的 MIG 摆放作为重叠 device 用 `consumesCounters` 从池里扣额度，调度器靠 counter 记账挑出不超分的合法组合。

```yaml
# DRA driver 自动发布的 ResourceSlice（H100，节选）
sharedCounters:
- name: gpu-0-counter-set            # 这张 GPU 的资源池（总量）
  counters:
    memory-slice-0: { value: "1" }   # 8 个显存切片，每格总量 1，充当占位 bit
    # ... slice-1 .. slice-7 ...
    multiprocessors: { value: "114" }
devices:
- name: gpu-0-mig-1g10gb-19-0        # 一个 1g 摆放
  attributes:
    profile: { string: 1g.10gb }
  consumesCounters:
  - counterSet: gpu-0-counter-set
    counters:
      memory-slice-0: { value: "1" } # 占第 0 格 → 与其它要 slice0 的设备互斥
      multiprocessors: { value: "14" }
```

`memory-slice-0..7` 这类总量为 1 的 counter 是物理占位 bit：占同一格的设备互斥，从而把 MIG 几何约束编码进调度。示例见 `dynamic-mig-h100/`（含真实 `resourceslice.yaml`）。

<Tip>
动态 MIG 只支持 **H100 及更新**（A100 无法在运行时自由开关 MIG 模式，会报 `SetMigMode ... Not Supported`）。`DynamicMIG` 与 `MPSSupport`、`NVMLDeviceHealthCheck`、`PassthroughSupport` 互斥。Kubernetes 1.33–1.35 还需在 apiserver/scheduler/kubelet 开启 `DRAPartitionableDevices` feature gate。
</Tip>

<Accordion title="练习入口：DRA 示例与一键部署">

本书仓库 `books/ai-systems-performance-engineering/chapters/ch03/dra/`：

- `quickstart/` — 整卡分配、容器/Pod 间共享
- `mps-timeslicing/` — TimeSlicing 与 MPS 软件级共享
- `static-mig-a100/` — `nvidia-mig-parted` 预切 + DRA 分配（A100 等）
- `dynamic-mig-h100/` — 按 `ResourceClaim` 动态切 MIG（H100+），含真实 `resourceslice.yaml`
- `partitionable-devices/` — `sharedCounters` / `consumesCounters` counter 机制示例
- `scripts/setup-dra-env.sh` — 幂等地装好 kind + DRA driver 实验环境
...
[Kubernetes Topology Manager](https://kubernetes.io/docs/tasks/administer-cluster/topology-manager/)在 kubelet admission 阶段汇总 CPU Manager、Device Manager 等 hint provider 的拓扑提示，把 CPU、设备、内存对齐到同一 NUMA 节点。常用策略：

| 策略 | 行为 | 适用场景 |
| --- | --- | --- |
| `none` | 不做拓扑对齐（默认） | 非性能敏感作业 |
...
Topology Manager 要和 [CPU Manager static policy](https://kubernetes.io/docs/tasks/administer-cluster/cpu-management-policies/) 配合：只有 Guaranteed Pod 且 CPU request 是整数时，CPU Manager 才会分配 exclusive CPUs，对齐才有意义。
...
但 Topology Manager 有两点局限：它在 **kubelet 准入阶段**（调度之后）工作，粒度只到 NUMA 节点；而且它的设备 hint 只来自 device plugin（Device Manager），表达不了“两张 GPU 是否 NVLink 直连”“多个 MIG 是否来自同一张卡”，DRA 设备它也看不到。

DRA 则在**调度阶段**直接表达设备间互联：driver 把拓扑写成设备属性（`resource.kubernetes.io/pcieRoot`、`gpu.nvidia.com/parentUUID`、`resource.kubernetes.io/numaNode` 等），claim 用 `constraints.matchAttribute` 要求选中的设备共享某属性，由 scheduler 跨集群强制满足。

```yaml
# 要求一组设备落在同一 PCIe root（换成 parentUUID 即“同一张物理卡”）
constraints:
- matchAttribute: "resource.kubernetes.io/pcieRoot"
```

两者互补：DRA 负责按互联拓扑挑对设备/节点；节点内 CPU 与 GPU 的 NUMA 对齐当前仍可借 Topology Manager / CPU Manager（DRA 原生纳管 CPU/内存自 Kubernetes 1.36 起为 alpha）。在 GB200 NVL72 这类系统上拓扑感知尤其关键，[NVIDIA GB200 NVL tuning guide](https://docs.nvidia.com/multi-node-nvlink-systems/multi-node-tuning-guide/overview.html) 说明其 rack 级 NVLink domain 的价值只有在调度和通信路径也对齐时才能体现；[NVIDIA Topograph](https://github.com/NVIDIA/topograph) 则提供了发现集群物理网络拓扑并暴露给调度器的工具。
...
#### 6.5 ComputeDomain：多节点 NVLink

节点内多张 GPU 走 NVLink/NVSwitch 直连即可；要让**不同节点**的 GPU 以全 NVLink 带宽共享显存，需要 Multi-Node NVLink（MNNVL）和 IMEX。NVIDIA DRA driver 的另一子系统 [ComputeDomain](https://dra-driver-nvidia-gpu.sigs.k8s.io/docs/)（`compute-domain.nvidia.com`）就是为此设计：当 workload 请求一个 `ComputeDomain` 时，driver 会按这组 Pod 所在节点临时编排 IMEX daemon、IMEX domain 和 channel device，随作业创建、随作业销毁。和仍处于 Technology Preview 的 GPU 分配侧不同，ComputeDomain 是 NVIDIA DRA driver 中**已正式支持**的部分。

前置条件：

- **MNNVL 硬件**：多个节点接入同一 NVLink 交换 fabric，典型是 GB200 NVL72。普通以太网 / InfiniBand 互联的双机不算。
- **软件栈**：Kubernetes `1.32+`，集群启用 DRA 和 CDI；NVIDIA Driver `v570.158.01+`。
- **节点标签与服务**：GPU Feature Discovery 给 MNNVL 节点打 `nvidia.com/gpu.clique` 标签；节点上禁用宿主机级 `nvidia-imex.service`，避免和 DRA driver 管理的 IMEX daemon 冲突。

用法是先建一个 `ComputeDomain`。最小配置只需要声明 channel 对应的 `ResourceClaimTemplate` 名称；`numNodes` 已废弃，推荐设为 `0`：
...
应用这个对象后，controller 会创建两类资源：

- 一个按 `ComputeDomain` 生成的 `compute-domain-daemon` DaemonSet，每个 GPU 节点上运行一个 daemon pod，用来包装和监督 `nvidia-imex`。
- 一个名为 `imex-channel-0` 的 `ResourceClaimTemplate`，workload Pod 通过它申请 IMEX channel。

Pod 通过 `resourceClaimTemplateName: imex-channel-0` 申请 channel 后，容器里会注入 `/dev/nvidia-caps-imex-channels/...` 和 IMEX mount；Pod 会等本地 IMEX daemon ready 后再启动：
...
默认 `spec.channel.allocationMode` 是 `Single`，只向 workload 注入一个 IMEX channel；需要访问 IMEX domain 内所有 channel 的 workload 可以设为 `All`。

验证时先看 `ComputeDomain` 状态，再看 clique 对象和 channel 注入结果：

```bash
kubectl get computedomain imex-channel-injection -o yaml
kubectl get computedomainclique -n nvidia-dra-driver-gpu
kubectl logs imex-channel-injection
```

<Tip>
ComputeDomain 只适用于真正的 MNNVL 系统（GB200 NVL72 等）。单张卡、单节点多卡、或普通网络互联的多机都用不上；这类场景下安装 DRA driver 时可 `--set resources.computeDomains.enabled=false` 关掉该子系统。
</Tip>

<Accordion title="ComputeDomain 参考资料">
...
#### 6.7 调度器：Kubernetes、Slurm 与 Slinky
...
容器性能问题通常不在 GPU 是否被“虚拟化”，而在运行时和数据路径是否配置正确。重点检查 driver/CUDA 版本、GPU device 注入、overlay FS、bind mount、网络路径和 NUMA 绑定。
</Accordion>
