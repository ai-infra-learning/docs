...
#### 6.6 Dynamic Resource Allocation（DRA）

device plugin 把 GPU 表达成可数的扩展资源（`nvidia.com/gpu`、`nvidia.com/mig-2g.45gb`），只能“要几个”，无法表达共享策略、按能力筛选或动态切分。[Dynamic Resource Allocation（DRA）](https://kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/) 从 Kubernetes v1.34 起 GA，把 GPU 管理改成类似 PV/PVC 的声明式模型：工作负载在 `ResourceClaim` 里声明“要什么”，由 [DRA Driver for NVIDIA GPUs](https://dra-driver-nvidia-gpu.sigs.k8s.io/docs/) 负责满足。一个节点上的 GPU 分配只能二选一：传统 device plugin 或 DRA driver。
...
用 DRA 申请一个 MIG 设备：`ResourceClaimTemplate` 用 `mig.nvidia.com` DeviceClass 加 CEL selector 按 profile 筛选，Pod 引用它即可：
...
相比 device plugin 的 `nvidia.com/mig-2g.45gb`，DRA 用属性 + selector 表达需求，还能在 claim 里附带共享策略（TimeSlicing、MPS）等配置，表达力更强。

动态 MIG 按 claim 现切。传统做法要先用 `nvidia-mig-parted` 预切，DRA 只分配已存在的 MIG 设备；从 DRA driver [`v25.12.0`](https://github.com/kubernetes-sigs/dra-driver-nvidia-gpu/releases/tag/v25.12.0) 起（alpha），开启 `DynamicMIG` feature gate 后，driver 可按 `ResourceClaim` 在卡上动态创建 MIG，无需预切。底层是 DRA 的 **partitionable devices**（[KEP-4815](https://github.com/kubernetes/enhancements/tree/master/keps/sig-scheduling/4815-dra-partitionable-devices)）：driver 把一张 GPU 的资源池声明成 `sharedCounters`，把所有可能的 MIG 摆放作为重叠 device 用 `consumesCounters` 从池里扣额度，调度器靠 counter 记账挑出不超分的合法组合。
...
`memory-slice-0..7` 这类总量为 1 的 counter 是物理占位 bit：占同一格的设备互斥，从而把 MIG 几何约束编码进调度，保证切出来的组合合法。
...
<Accordion title="练习入口：DRA + MIG / 共享示例">

本书仓库 `books/ai-systems-performance-engineering/chapters/ch03/dra/` 提供可直接运行的 DRA 示例与一键部署脚本：
